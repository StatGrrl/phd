# -*- coding: utf-8 -*-
"""
Created on Sat Feb 6 12:14:00 2016

@author: Lisa

An adaptation to the null game which assumes that the poacher will not leave immediately if
he has not yet poached a rhino

In the null game, the poacher leaves if he enters a border cell. Since he starts in a
border cell, the adjacent cell which he moves into may also be a border cell, in
which case the game ends in the first move. This seems unrealistic - 
why would a poacher risk entering the area if he intends to leave so quickly? 
One solution would be to prohibit him from moving along the border. However, this might
be realistic behaviour - staying close to the border until a rhino is observed is
safer behaviour. 
Some kind of time constraint is proposed. For example, he may only leave after 10 or more moves. 
How to determine the number of moves remains a challenge. Realistically the poacher may leave for
various reasons. For example, he may grow tired, might injure himself, there may be 
bad weather conditions, he can't see in the dark, might have other obligations to attend to, etc.
A randomization over a vector representing the number of moves (10, 11, . . . , 30) seems like
a reasonable way to account for such behaviour.
